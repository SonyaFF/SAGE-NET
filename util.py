...
def get_ehr_adj(records, Nmed, Npat, no_weight=True, filter_th=None) -> np.array:
    ehr_adj = np.zeros((Nmed + Npat, Nmed + Npat))

    for pat_idx, patient in enumerate(records):
        pat_idx += Nmed
        for adm in patient:
            med_set = adm[2]
            for med_i in med_set:
                ehr_adj[pat_idx, med_i] = 1
                ehr_adj[med_i, pat_idx] = 1
   ...
   # Save the ehr_adj matrix to a file
    if not os.path.exists(args.datadir):
      os.makedirs(args.datadir) 
    ehr_adj_path = os.path.join(args.datadir, 'ehr_adj2_new.pkl')
    with open(ehr_adj_path, 'wb') as f:
        dill.dump(ehr_adj, f)             
...
def ddi_rate_score(record, path):
    # ddi rate
    if isinstance(path, str):
        path = '/content/drive/MyDrive/Carmen-main/data/ddi_A_final_4.pkl' 
        ddi_A = dill.load(open(path, 'rb'))
    else:
        ddi_A = path
    all_cnt = 0
